...
namespace DutyMechanic.Dungeons
{
    ...
    internal static class ThirdBoardGeometry
    {
        ...
        internal static bool InArena(Vector2 p, uint boss, float inset = .5f)
        {
            ...
            if (boss == 0x4CAA)
            {
                ...
                return Floor(p) && Enumerable.Range(0, 16).All(i =>
                    Floor(p + Direction(i * MathF.PI / 8) * inset));
            }
        ...
        private static bool Floor(Vector2 p) => Box(p, 520, -420, 10, 20) ||
            Box(p, 520, -397.5f, 2.5f, 2.5f) || Box(p, 520, -442.5f, 2.5f, 2.5f) ||
            Box(p, 508, -412.5f, 2.5f, 2.5f) || Box(p, 508, -422.5f, 2.5f, 2.5f) ||
            Box(p, 532, -417.5f, 2.5f, 2.5f) || Box(p, 532, -427.5f, 2.5f, 2.5f);

        private static bool Box(Vector2 p, float x, float z, float halfX, float halfZ) =>
            Math.Abs(p.X - x) <= halfX && Math.Abs(p.Y - z) <= halfZ;

        internal static Vector2 Center(uint boss) => boss == 0x4CAA ? new Vector2(520, -420) :
            boss == 0x4C9B || boss == 0x4CA1 ? new Vector2(120, -420) : new Vector2(120, 0);
        ...
        internal static Vector2? PreferMeleeRefuge(Vector2? held, Vector2 current, Vector2 target,
            float range, IEnumerable<Vector2> candidates, Func<Vector2, bool> safe,
            Func<Vector2, Vector2, bool> corridor)
        {
            ...
            return ring.Concat(candidates).Where(p => Melee(p) && safe(p) && corridor(current, p))
                .OrderBy(p => Vector2.Distance(p, current)).Select(p => (Vector2?)p).FirstOrDefault();
        }
        ...
        internal static ThirdBoardHazard[] CageForecast(uint actor, Vector2 origin, float heading, DateTime until) =>
            new[] { heading, heading + MathF.PI / 2 }.Select(angle => new ThirdBoardHazard
            {
                Actor = actor,
                Action = 48611,
                Origin = origin,
                Heading = angle,
                Points = ThirdBoardHazard.Rectangle(5.5f, 15.5f, 15.5f),
                Until = until,
                Persistent = true
            }).ToArray();
        ...
        internal static bool LandslipMoved(Vector2 before, Vector2 after, float heading)
        {
            ...
            return Vector2.Dot(delta, direction) >= 3 &&
                Math.Abs(delta.X * direction.Y - delta.Y * direction.X) < 3;
        }
        ...
        internal static bool SirenRelocated(Vector2 before, Vector2 after, double seconds) =>
            seconds > 0 && seconds <= .5 && Vector2.Distance(before, after) > 6 &&
            Vector2.Distance(after, Center(0x4CA1)) > 14;
        ...
        internal static Vector2 SirenEdgeOrigin(Vector2 position) =>
            Center(0x4CA1) + Vector2.Normalize(position - Center(0x4CA1)) * 21;

        internal static bool MelodyRefuge(Vector2 point, Vector2 origin, ThirdBoardHazard[] hazards, bool prepareMarch)
        {
            if (!InArena(point, 0x4CA1) || Vector2.Distance(point, origin) > 12 ||
                hazards.Any(h => h.Contains(point)))
            {
                return false;
            }

            if (!Enumerable.Range(0, 8).All(i => hazards.All(h =>
                !h.Contains(point + Direction(i * MathF.PI / 4) * .6f))))
            {
                return false;
            }
        ...
        internal static IEnumerable<Vector2> Candidates(uint boss)
        {
        ...
        internal static bool WaterEscapeCorridor(Vector2 start, Vector2 end, ThirdBoardHazard[] hazards)
        {
            ...
            {
                if (hazard.Action != 48483 || Vector2.Distance(start, hazard.Origin) > .25f &&
                    Vector2.Dot(start - hazard.Origin, end - start) < -.01f)
                {
                    return false;
                }
            }
    ...
    internal sealed class ThirdBoardHazard
    {
        ...
        internal static Vector2[] Circle(float radius) => Enumerable.Range(0, 96)
            .Select(i => ThirdBoardGeometry.Direction(i * MathF.PI / 48) * radius).ToArray();

        internal static Vector2[] Rectangle(float halfWidth, float length, float back = .5f) => new[]
        {
            new Vector2(-halfWidth, -back), new Vector2(halfWidth, -back),
            new Vector2(halfWidth, length), new Vector2(-halfWidth, length)
        };

        internal static Vector2[] Cone(float radius, float halfAngle) => new[] { Vector2.Zero }
            .Concat(Enumerable.Range(0, 65).Select(i => ThirdBoardGeometry.Direction(
                (-halfAngle + halfAngle * 2 * i / 64) * MathF.PI / 180) * radius)).ToArray();
        ...
        internal static IEnumerable<Vector2[]> Donut(float inner, float outer)
        {
            for (int i = 0; i < 64; i++)
            {
                ...
                yield return new[] { a * inner, a * outer, b * outer, b * inner };
            }
        }
    }
}
